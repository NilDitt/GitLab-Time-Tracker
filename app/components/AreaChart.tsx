    ...
    <div style={{ ...styles.container, height }}>
        ...
        {showPoints &&
          prepared.points.map((point) => (
            <g key={`${point.date}-${point.value}`}>
              ...
              />
              <title>
                {point.label}: {point.value} commit
                {point.value === 1 ? "" : "s"}
              </title>
            </g>
          ))}
      ...
      <div style={styles.labels}>
        {prepared.points.map((point, index) => {
          ...
          const shouldShow =
            prepared.points.length <= 7 ||
            index % Math.ceil(prepared.points.length / 7) === 0;
          return shouldShow ? (
            <span
      ...
      <div style={styles.summary}>
        ...
        <span style={styles.summaryItem}>
          Avg:{" "}
          {(
            prepared.points.reduce((sum, p) => sum + p.value, 0) /
...
function createSmoothCurvePath(
  ...
  if (points.length > 1) {
    path.push(
      `Q ${secondLastPoint.x} ${secondLastPoint.y} ${lastPoint.x} ${lastPoint.y}`
    );
  }
